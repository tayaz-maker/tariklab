...
function actorPanel(s, axis, nameOf) {
  ...
  const options = Object.keys(DIPLOMACY)
    .map((kind) => {
      ...
      return `<article class="dip-option"><h4>${t(...KIND[kind].label)}</h4><p class="muted">${t(...KIND[kind].note)}</p><ul class="geo-effects">${rel}</ul><p class="geo-mini">${world}${p.heat ? ` · ${t("İç gerilim", "Domestic tension")} ${signed(p.heat)}` : ""}</p><p class="geo-mini dip-later">${t(`${p.dueIn} ay sonra`, `In ${p.dueIn} months`)}: ${laterText}</p><button type="button" class="geo-act" data-diplo="${axis}:${kind}" ${p.ok ? "" : "disabled"}>${t("Başlat", "Launch")} · ${p.cost} ${t("kapasite", "capacity")}${p.ok ? "" : ` · ${h(t(...p.reason))}`}</button></article>`;
    })
    .join("");
  return `<div class="geo-panel dip-panel"><div class="geo-panel__main"><p class="eyebrow">${t("SEÇİLİ TARAF", "SELECTED PARTY")}</p><h3>${h(nameOf(axis))}</h3><div class="geo-row"><span>${t("İlişki endeksi", "Relation index")}</span><b>${n(v)} · ${t(...relBand(v))}</b><i style="--v:${v}%;--c:${tone(v)}"></i></div><p class="geo-mini">${ties.rivals.length ? `${t("Yakınlaşmaya tepki verenler", "React against closer ties")}: ${ties.rivals.map((k) => h(nameOf(k))).join(", ")}. ` : ""}${ties.partners.length ? `${t("Birlikte ısınanlar", "Warm alongside")}: ${ties.partners.map((k) => h(nameOf(k))).join(", ")}.` : ""}</p>${pending.length ? `<p class="geo-status is-set">${pending.map((p) => `◆ ${t(...KIND[p.kind].label)} · ${t(`${p.due - s.time.turn} ay içinde sonuç`, `result in ${p.due - s.time.turn} months`)}`).join("<br>")}</p>` : ""}<p class="geo-mini">${t("Endeks oyunun kendi ilişki ölçüsüdür; gerçek bir ülkenin tutumu hakkında iddia değildir.", "The index is the game's own relation measure, not a claim about any real country's position.")}</p></div><div class="dip-options">${options}</div></div>`;
}
