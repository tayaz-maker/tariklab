...
function draw(session) {
  ...
  view = "menu";
  const month = s.month;
  root.innerHTML = `<main class="game-root town-root"><header class="topbar town-command"><a href="/">${t("← Oyunlar", "← Games")}</a><strong class="town-command__identity">${h(s.name)} · ${t("AY", "MONTH")} ${s.month}/24</strong><div class="town-command__metrics"><span>${t("Bütçe", "Budget")} <b>${number(s.budget)} TL</b></span><span>${t("Nüfus", "Population")} <b>${number(population(s))}</b></span><span>${t("Borç", "Debt")} <b>${number(s.debt)} TL</b></span><span>${t("Kapasite", "Capacity")} <b>${s.capacityUsed || 0}/${s.capacityMax || 10}</b></span></div><div class="topbar__tools">${savePanel(session)}<button id="town-menu">${t("Menü", "Menu")}</button><button id="town-advance" ${s.ended ? "disabled" : ""}>${s.ended ? t("TAMAMLANDI", "COMPLETE") : t("AYI KAPAT", "CLOSE MONTH")}</button></div></header><header class="town-masthead"><div><p class="eyebrow">${t("KÖY DEFTERİ", "VILLAGE LEDGER")} · ${t("AY", "MONTH")} ${s.month}/24</p><h1>${h(s.name)}</h1><p>${h(tr(IDENTITIES[s.identity]))}</p></div><div class="town-totals"><span>${t("Bütçe", "Budget")}<b>${number(s.budget)} TL</b></span><span>${t("Nüfus", "Population")}<b>${number(population(s))}</b></span><span>${t("Borç", "Debt")}<b>${number(s.debt)} TL</b></span></div></header><div class="town-layout"><nav class="town-nav" aria-label="${t("Köy bölümleri", "Village sections")}">${NAV.map((n) => `<button type="button" data-screen="${n[0]}" class="${s.ui.screen === n[0] ? "is-active" : ""}">${t(n[1], n[2])}</button>`).join("")}</nav><section class="town-content"><p class="notice" role="status">${h(session.notice)}</p><div id="town-panel">${townPanel(s)}</div>${help()}</section><aside class="town-feed"><details open><summary>${t("SONUÇ AKIŞI", "CONSEQUENCE FEED")}</summary>${history(s)}</details></aside></div><footer>© 2026 TarikLab · Tarık Halil Ayaz</footer></main>`;
  root
