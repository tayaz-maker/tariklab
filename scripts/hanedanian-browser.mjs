...
try {
 ...
 await importState(page,seeded);await page.waitForFunction(()=>document.querySelector('#settlement-rail').textContent.includes('Çevrimdışı paket hazır'),null,{timeout:30000});await page.locator('#menu-button').click();await page.locator('#dialog').waitFor({state:'visible'});assert.match(await page.locator('#dialog').innerText(),/Çevrimdışı paket hazır/);await page.locator('[data-action="resume"]').click();await save(page);
