...
try {
 ...
 for(const [width,height,mobile] of [[1280,720,false],[1920,1080,false],[360,800,true]]){
  ...
  if(mobile){
   ...
   await page.goto(`${origin}/credits.html`);await page.goBack();await page.locator('[data-load="auto"]').click();await checkLayout(page,'mobile:back');
  }
  await context.close();
 }
 // Real SW + real IndexedDB; assert an uncached request fails while offline.
 const context=await browser.newContext({viewport:{width:360,height:800},isMobile:true,hasTouch:true});const page=await context.newPage();page.on('pageerror',e=>errors.push(String(e)));
 ...
 const exported=await archive(page);await context.setOffline(true);await page.reload();await page.locator('[data-load="auto"]').click();assert.equal((await archive(page)).world.seed,exported.world.seed);
 ...
 await page.reload();await page.locator('[data-load="auto"]').click();assert.equal((await archive(page)).settlements[0].queue.length,changed.settlements[0].queue.length);
 await page.locator('#menu-button').click();await page.locator('[data-load="manual"]').click();assert.equal((await archive(page)).world.seed,'TL-OFFLINE-REAL');
 ...
 await page.goto(`${origin}/games/hanedanian/index.html`);await page.locator('[data-load="auto"]').click();assert.equal((await archive(page)).world.seed,'TL-OFFLINE-REAL');
 await context.setOffline(false);await page.reload();await page.locator('[data-load="auto"]').click();await checkLayout(page,'offline:reconnect');await page.screenshot({path:`out/offline.png`.replace('out/',out+'/')});
