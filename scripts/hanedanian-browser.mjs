...
async function importState(page,state){if(await page.locator('#welcome').isVisible())await page.locator('#welcome [data-action="import"]').click();else{await page.locator('#menu-button').click();await page.locator('#dialog [data-action="import"]').click();}await page.locator('#import-text').fill(encodeSave(state));await page.locator('#import-form button[type="submit"]').click();await page.locator('#dialog').waitFor({state:'hidden'});await page.locator('#welcome').waitFor({state:'hidden'});}
try {
 ...
 await importState(page,seeded);await page.waitForFunction(()=>{const text=document.querySelector('#settlement-rail')?.textContent||'';return text.includes('Çevrimdışı paket hazır')&&!text.includes('hazırlanıyor');},null,{timeout:30000});await page.locator('#menu-button').click();await page.locator('#dialog').waitFor({state:'visible'});assert.match(await page.locator('#dialog').innerText(),/Çevrimdışı paket hazır\./);await page.locator('[data-action="resume"]').click();await save(page);
 ...
 await page.locator('#navigation [data-view="settlement"]').click();await page.locator('[data-build="lumber"]').click();await page.locator('[data-build="quarry"]').click();await save(page);const changed=await archive(page);assert.ok(changed.settlements[0].queue.length>0);
 ...
 await page.locator('#menu-button').click();await page.locator('#dialog [data-action="import"]').click();await page.locator('#import-text').fill('{"bad":"save"}');await page.locator('#import-form button').click();await page.locator('#dialog-notice').waitFor({state:'visible'});assert.ok(await page.locator('#dialog-notice').isVisible());await page.locator('#dialog').press('Escape');assert.equal((await archive(page)).world.seed,'TL-OFFLINE-REAL');
