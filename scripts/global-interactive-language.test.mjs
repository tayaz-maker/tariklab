...
test("Resources exposes live JITEM and preserves the personal note in both languages", async () => {
  ...
  assert.match(credits, /data-game="jitem-derin-ag"[^>]*data-status="live"[^>]*data-route="\/oyna\/jitem-derin-ag"/);
  assert.match(credits, /Kaynaklı tarih kayıtları/);
  assert.match(globalI18n, /sourced historical records/);
});
