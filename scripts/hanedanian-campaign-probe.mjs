...
export function runCampaign(seed, path, {limit=100000, policy='network', snapshot=false}={}) {
  ...
  const log={seed,path,policy,commands:0,commandTypes:{},rejected:0,stages:[],early:false,losses:0,maxAITowns:1,maxAIShare:0};
  let lastStage=0, previousTownCount=1;
  const order=a=>{const r=dispatch(s,a); if(r.ok){log.commands++;log.commandTypes[a.type]=(log.commandTypes[a.type]||0)+1;} else log.rejected++; return r.ok;};
  const send=(t,type,extra)=>order({type,settlementId:t.id,...extra});
  const sitesFor=r=>s.world.tiles.filter(z=>!z.poi&&regionOf(s,z)===r&&s.world.tiles.filter(q=>q.poi&&regionOf(s,q)===r&&distance(q,z)<=7).length>=2);
  ...
  for (let time=0; time<limit; time+=120) {
    ...
    if(towns.length+ongoing.length<(policy==='expansion'?12:Math.max(8,group.size+2)) && policy!=='idle') {
    ...
    for(const t of towns) {
      ...
      if(policy==='circular' && towns.length>1 && t.buildings.market){const target=towns[(towns.indexOf(t)+1)%towns.length];send(t,'trade',{targetId:target.id,cargo:{food:25,wood:25,stone:25,iron:25}});}
...
if(import.meta.url===pathToFileURL(process.argv[1]).href) {
  ...
  for(let i=0;i<count;i++)for(const policy of ['idle','expansion','circular']){const r=runCampaign(`TL-CAMPAIGN-${i}`,'wealth',{limit:12000,policy});exploits.push(r);writeFileSync((process.env.PROBE_OUTPUT||'/tmp/hanedanian-campaign-probe.json').replace('.json','-exploits.json'),JSON.stringify(exploits,null,2));}
  console.log(JSON.stringify({campaigns:results.length,exploits:exploits.length,exploitWins:exploits.filter(r=>r.victory).length}));
  if(results.some(r=>!r.victory||r.early||!r.resourcesFinite)||exploits.some(r=>r.victory||!r.resourcesFinite||r.maxAITowns>7||r.towns>12))process.exitCode=1;
}
